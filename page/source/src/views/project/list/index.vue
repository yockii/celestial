...
import { getProjectList, addProject, getProjectStageStatistics } from "@/service/api"
...
// 计算时间
const timeBefore = computed(() => (t: number) => dayjs(t).fromNow())
</script>
          ...
          <n-card embedded size="small">
            <n-grid :cols="20" x-gap="8" y-gap="8">
              ...
              <n-gi :span="8" class="flex flex-justify-end">
                <n-text depth="3"> 已用工时: 0小时 </n-text>
              </n-gi>
              <n-gi :span="20">
                <n-ellipsis>{{ project.description }}</n-ellipsis>
              </n-gi>
              <n-gi :span="4">
                <n-text depth="3"> 创建时间: {{ timeBefore(project.createTime as number) }} </n-text>
              </n-gi>
              <n-gi :span="4">
                <n-text depth="3"> 当前阶段: {{ getStageName(project.stageId) }} </n-text>
              </n-gi>
          ...
          <n-card embedded size="small">
            ...
            <n-grid :cols="3" class="mt-8px">
              <n-gi @click="stageSelected('all')">
                ...
                <n-text tag="div" class="cursor-pointer font-500 text-2.5em w-full pl-20px" :type="selectedStageId == 'all' ? 'success' : 'default'">{{
                  total
                }}</n-text>
              </n-gi>
              <n-gi v-for="stage in stageList" :key="stage.id" @click="stageSelected(stage.id)">
                <n-text class="cursor-pointer list-item ml-20px text-1em text-gray" :type="selectedStageId == stage.id ? 'success' : 'default'">{{
                  stage.name
                }}</n-text>
