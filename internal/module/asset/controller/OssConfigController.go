...
func (c *ossConfigController) Add(ctx *fiber.Ctx) error {
	...
	duplicated, success, err := service.OssConfigService.Add(instance)
	if err != nil {
		return ctx.JSON(&server.CommonResponse{
...
func (c *ossConfigController) Update(ctx *fiber.Ctx) error {
	instance := new(model.OssConfig)
	if err := ctx.BodyParser(instance); err != nil {
		logger.Errorln(err)
		return ctx.JSON(&server.CommonResponse{
	...
	success, err := service.OssConfigService.Update(instance)
	if err != nil {
		return ctx.JSON(&server.CommonResponse{
			Code: server.ResponseCodeDatabase,
			Msg:  server.ResponseMsgDatabase + err.Error(),
...
func (c *ossConfigController) Delete(ctx *fiber.Ctx) error {
	instance := new(model.OssConfig)
	...
	success, err := service.OssConfigService.Delete(instance.ID)
	if err != nil {
		return ctx.JSON(&server.CommonResponse{
			Code: server.ResponseCodeDatabase,
			Msg:  server.ResponseMsgDatabase + err.Error(),
...
func (c *ossConfigController) List(ctx *fiber.Ctx) error {
	instance := new(domain.OssConfigListRequest)
	if err := ctx.QueryParser(instance); err != nil {
		logger.Errorln(err)
		return ctx.JSON(&server.CommonResponse{
			Code: server.ResponseCodeParamParseError,
	...
	total, list, err := service.OssConfigService.PaginateBetweenTimes(&instance.OssConfig, paginate.Limit, paginate.Offset, instance.OrderBy, tcList)
	if err != nil {
		return ctx.JSON(&server.CommonResponse{
			Code: server.ResponseCodeDatabase,
			Msg:  server.ResponseMsgDatabase + err.Error(),
...
func (c *ossConfigController) Instance(ctx *fiber.Ctx) error {
	condition := new(model.OssConfig)
	...
	dept, err := service.OssConfigService.Instance(condition.ID)
	if err != nil {
		return ctx.JSON(&server.CommonResponse{
			Code: server.ResponseCodeDatabase,
			Msg:  server.ResponseMsgDatabase + err.Error(),
